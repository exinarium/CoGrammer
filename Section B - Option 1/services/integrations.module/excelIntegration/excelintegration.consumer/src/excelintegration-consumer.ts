...
import { ExcelIntegrationRepository } from './domain/repositories/excelintegration-repository';
import Configuration from './configuration.json';
import { ResponseStatus, ExcelIntegrationMessage } from '../../excelintegration.dto/src';
...
export class ExcelIntegrationConsumer{
    ...
    constructor() {
        const databaseConfigs = new Array<DBConfig>();

        Configuration.databaseConfigs.forEach(element => {
            databaseConfigs.push(element);
        });
        ...
        this.config = new Config(messageBusConfig, databaseConfigs);
        const repository = new ExcelIntegrationRepository(this.config);
        this.aggregate = new ExcelIntegrationAggregate(repository);
    ...
    async consumeAsync(id: ObjectID, message: ExcelIntegrationMessage<ExcelIntegrationDataModel>): Promise<number> {
        try {
            ...
            const ERROR_MESSAGE = 'Error occurred while consuming message in ExcelIntegration consumer';
